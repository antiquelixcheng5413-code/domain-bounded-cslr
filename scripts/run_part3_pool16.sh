#!/usr/bin/env bash
# Token-density sweep down K=16 (tri + pooling). Frozen settings identical to
# run_part3_pool.sh, only num_pool_tokens=16.
set -euo pipefail
cd "$(dirname "$0")/.."
export PYTHONUNBUFFERED=1

PYTHONPATH=src venv/bin/python -m cslr.translation train-real \
  --config configs/translation_part3_pool16.yaml \

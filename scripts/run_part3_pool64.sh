#!/usr/bin/env bash
# Token-density sweep K=64 (tri + pooling). Frozen settings identical to
# run_part3_pool.sh, only num_pool_tokens=64.
set -euo pipefail
cd "$(dirname "$0")/.."

PYTHONPATH=src venv/bin/python -m cslr.translation train-real \
  --config configs/translation_part3_pool64.yaml \
